...
def simulate_environmental_conditions(image):
    """
    Simulate environmental conditions such as lighting changes to enhance image diversity.
    """
    augmented_image = transforms.ColorJitter(brightness=0.3, contrast=0.3)(image)
...
def attack(idx,true_label,initial_pic,sticker,opstickercv,magnification,\
    cleancrop,zstore,target=None, maxiter=50, popsize=60):
    
    
    initial_pic = simulate_environmental_conditions(initial_pic)
...
# Define global variables to save idx that is not interrupted
successful_idx = []
...
# Decorator: used to set function timeout
...
# attack function: simulate long time calculation
@timeout_decorator(120)  # Set the timeout to 120 seconds
def perform_attack(idx, true_label, initial_pic, sticker, opstickercv, magnification, cleancrop, zstore):
    return attack(idx, true_label, initial_pic, sticker, opstickercv, magnification, cleancrop, zstore)
...
if __name__=="__main__":
    ...
    #     result_huhuhuhuhuu = attack(idx,true_label, initial_pic, sticker,opstickercv,magnification,cleancrop,zstore)
    #     print(result_huhuhuhuhuu)
    ...
    # Read stickers and preprocess them
    stickerpath = './stickers/{}.png'.format(opt.sticker_name)
    stickerpic = Image.open(stickerpath)
    scale1 = stickerpic.size[0] // 23
    scale2 = opt.scale
    ...
    idx_list = random.sample(range(len(opt.dataset)), 10)
    ...
    for idx in idx_list:
        ...
        # Initial prediction and check if labels match
        rank, _, cleancrop = eval(f'predict.initial_predict_{threat_model}([initial_pic])')
        if rank[0][0] == true_label:
            zstore = mapping3d.generate_zstore(initial_pic)
            ...
            # Try to perform the attack and monitor the time
            result = perform_attack(idx, true_label, initial_pic, sticker, opstickercv, magnification, cleancrop, zstore)

            # Check attack results
            if result is not None:
                # If the attack is not interrupted, save the idx
                successful_idx.append(idx)
                print(f"Attack succeeded for idx: {idx}, time elapsed: {time.time() - start_time:.2f} seconds")
            else:
                print(f"Attack for idx {idx} was interrupted due to timeout.")

    # Output the uninterrupted idx list
    print("Indices of successful attacks (without timeout):", successful_idx)
